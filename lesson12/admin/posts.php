                ...
                <div class="row">
                    ...
                    <div class="col-lg-12">
                        ...
                        if (isset($_GET["active"])){
                            if ($_GET["active"] == "create") {                                               //СОЗДАНИЕ ПОСТА
                                ?>
                            ...
                            }elseif ($_GET["active"] == "update"){                                            //РЕДАКТИРОВАНИЕ ПОСТА
                                $currentId = $_GET['id'];
                                ...
                                <form action="index.php?page=posts" method="POST" enctype="multipart/form-data" class="form">
                                    ...
                                    <label for="content" class="col-lg-6">Content</label>
                                    <input type="text" name="content" class="form-control" value="<?= $currentContent?>">
                                    <label for="image" class="col-lg-6">Image</label>
                                    <input type="file" name="image" class="form-control">
                                    <label for="categoty" class="col-lg-6">Category</label>
                                    <input type="text" name="category" class="form-control" value="<?= $currentCategory?>">
                                ...
                                if (isset($_POST['postUpdate'])){
                                    ...
                                    move_uploaded_file($_FILES["image"]["tmp_name"], $postImage);
                                    echo $_FILES["image"]["tmp_name"];
                                    $updPost = "UPDATE posts SET title = '{$postTitle}', author = '{$postAuthor}', created_at = NOW(), content = '{$postContent}', image = '{$postImage}', cat_id = '{$postCategory}' WHERE id = '{$currentId}'";
                                    $updPostRezult = mysqli_query($connection, $updPost);
                                    if(!$updPostRezult) {
                                        echo "Запрос не удался";
                                    }
                                }
                            }elseif ($_GET["active"] == "delete") {         //УДАЛЕНИЕ ПОСТА
                                $currentId = $_GET['id'];
